...
CREATE FUNCTION "public"."kw_match_documents"("query_text" "text", "match_count" integer) RETURNS TABLE("id" bigint, "content" "text", "metadata" "jsonb", "similarity" real)
    LANGUAGE "plpgsql"
    AS $_$
...
CREATE FUNCTION "public"."match_documents"("query_embedding" "public"."vector", "match_count" integer) RETURNS TABLE("id" bigint, "content" "text", "metadata" "jsonb", "similarity" double precision)
    LANGUAGE "plpgsql"
    AS $$
...
CREATE SEQUENCE "public"."chunks_document_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE "public"."chunks_document_id_seq" OWNER TO "postgres";

ALTER SEQUENCE "public"."chunks_document_id_seq" OWNED BY "public"."chunks"."document_id";
...
ALTER TABLE ONLY "public"."chunks"
    ADD CONSTRAINT "chunks_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "Chunks are viewable by users who created them." ON "public"."chunks" FOR SELECT USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Documents are viewable by users who created them." ON "public"."documents" FOR SELECT USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can only see their own profile" ON "public"."users" FOR SELECT USING (("auth"."uid"() = "id"));

CREATE POLICY "Users can only delete their own chunks" ON "public"."chunks" FOR DELETE USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can only delete their own documents" ON "public"."documents" FOR DELETE USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can only insert their own chunks" ON "public"."chunks" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can only insert their own documents" ON "public"."documents" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can only update their own chunks" ON "public"."chunks" FOR UPDATE USING (("auth"."uid"() = "user_id"));

CREATE POLICY "Users can only update their own documents" ON "public"."documents" FOR UPDATE USING (("auth"."uid"() = "user_id"));
...
GRANT ALL ON SEQUENCE "public"."chunks_document_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."chunks_document_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."chunks_document_id_seq" TO "service_role";
